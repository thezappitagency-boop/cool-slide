...
(function ($) {
		...
		if ($('.tp--hover-item').length) {
			...
			let hoverAnimation = function () {
				$(".tp--hover-img").each(function () {
					let n = $(this);
					if (n.data("hoverInitialized")) return;
					n.data("hoverInitialized", true);
	...
	// last child menu
	$('.tp-agntix-default-header nav > ul > li').slice(-4).addClass('menu-last');
	...
	$(window).on("elementor/frontend/init", function () {
		...
		elementorFrontend.hooks.addAction(
			"frontend/element_ready/tp-project-box.default", animation_images
		);
	...
	$("a:has(img[alt='Seraphinite Accelerator'])").hide()
