...
<p class="woocommerce-result-count" <?php echo ( empty( $orderedby ) || 1 === intval( $total ) ) ? '' : 'role="alert" aria-relevant="all" data-is-sorted-by="true"'; ?>>
	...
	if ( 1 === intval( $total ) ) {
		_e( 'Showing the single result', 'agntix' );
		...
		printf( _n( 'Showing all %1$d result', 'Showing all %1$d results', $total, 'agntix' ) . $orderedby_placeholder, $total, esc_html( $orderedby ) );
	} else {
		...
		printf( _nx( 'Showing %1$d&ndash;%2$d of %3$d result', 'Showing %1$d&ndash;%2$d of %3$d results', $total, 'with first and last result', 'agntix' ) . $orderedby_placeholder, $first, $last, $total, esc_html( $orderedby ) );
	}
