...
<form class="cart grouped_form d-block" action="<?php echo esc_url( apply_filters( 'woocommerce_add_to_cart_form_action', $product->get_permalink() ) ); ?>" method="post" enctype='multipart/form-data'>
	<table cellspacing="0" class="woocommerce-grouped-product-list group_table mb-20">
		<tbody>
			<?php
							...
							if ( ! $grouped_product_child->is_purchasable() || $grouped_product_child->has_options() || ! $grouped_product_child->is_in_stock() ) {
								...
								if ( $grouped_product_child->is_on_sale() ) {
									printf(
										...
										esc_html__( 'Buy one of %1$s on sale for %2$s, original price was %3$s', 'agntix' ),
									...
									printf(
										...
										esc_html__( 'Buy one of %1$s for %2$s', 'agntix' ),
										esc_html( $grouped_product_child->get_name() ),
										esc_html( wp_strip_all_tags( wc_price( $grouped_product_child->get_price() ) ) )
									);
								}
	...
	<?php if ( $quantites_required && $show_add_to_cart_button ) : ?>
		...
		<button type="submit" class="tp-product-details-add-to-cart-btn w-100 single_add_to_cart_button button alt<?php echo esc_attr( wc_wp_theme_get_element_class_name( 'button' ) ? ' ' . wc_wp_theme_get_element_class_name( 'button' ) : '' ); ?>"><?php echo esc_html( $product->single_add_to_cart_text() ); ?></button>
		</div>
		<?php do_action( 'woocommerce_after_add_to_cart_button' ); ?>

	<?php endif; ?>
</form>
<?php do_action('agntix_woocommerce_buy_now_button'); ?>
